...
| 設定 | デフォルト | 説明 |
|------|----------|------|
| `inlineMarkdownEditor.sync.debounceMs` | `300` | 変更を同期するまでの遅延（ミリ秒） |
| `inlineMarkdownEditor.sync.timeoutMs` | `5000` | エラー表示までの同期タイムアウト |
| `inlineMarkdownEditor.changeGuard.maxDiffRatio` | `0.5` | 最大許容変更率（0-1） |
| `inlineMarkdownEditor.changeGuard.minLength` | `100` | 変更ガードの最小長 |

#### セキュリティ設定

| 設定 | デフォルト | 説明 |
|------|----------|------|
| `inlineMarkdownEditor.security.renderHtml` | `false` | HTMLブロックをレンダリング（DOMPurifyでサニタイズ） |
| `inlineMarkdownEditor.security.allowRemoteImages` | `false` | リモート画像の読み込みを許可 |
| `inlineMarkdownEditor.security.allowInsecureRemoteImages` | `false` | HTTP（非HTTPS）画像を許可 |
| `inlineMarkdownEditor.security.confirmExternalLinks` | `true` | 外部リンクを開く前に確認を表示 |

#### ログ設定

| 設定 | デフォルト | 説明 |
|------|----------|------|
| `inlineMarkdownEditor.log.level` | `"info"` | ログレベル: debug, info, warn, error |
| `inlineMarkdownEditor.log.enableFileLog` | `false` | JSONLファイルログを有効化 |
| `inlineMarkdownEditor.log.maxFileSizeKB` | `1024` | ローテーション前の最大ログファイルサイズ |
| `inlineMarkdownEditor.log.retentionDays` | `7` | 古いログファイルを保持する日数 |

### 📋 コマンド

| コマンド | 説明 |
|---------|------|
| `inlineMarkdownEditor.resetSession` | エディタセッションをリセットして再同期 |
| `inlineMarkdownEditor.applyRequiredSettings` | 推奨ワークスペース設定を適用 |
| `inlineMarkdownEditor.exportLogs` | デバッグ用にログをエクスポート |
...
#### 変更が同期されない
1. 出力パネルでエラーを確認（表示 > 出力 > Inline Markdown Editor）
...
| 设置 | 默认值 | 描述 |
|------|--------|------|
| `inlineMarkdownEditor.sync.debounceMs` | `300` | 同步更改前的延迟（毫秒） |
| `inlineMarkdownEditor.sync.timeoutMs` | `5000` | 显示错误前的同步超时 |
| `inlineMarkdownEditor.changeGuard.maxDiffRatio` | `0.5` | 最大允许更改比率（0-1） |
| `inlineMarkdownEditor.changeGuard.minLength` | `100` | 更改保护的最小长度 |

#### 安全设置

| 设置 | 默认值 | 描述 |
|------|--------|------|
| `inlineMarkdownEditor.security.renderHtml` | `false` | 渲染 HTML 块（使用 DOMPurify 净化） |
| `inlineMarkdownEditor.security.allowRemoteImages` | `false` | 允许加载远程图片 |
| `inlineMarkdownEditor.security.allowInsecureRemoteImages` | `false` | 允许 HTTP（非 HTTPS）图片 |
| `inlineMarkdownEditor.security.confirmExternalLinks` | `true` | 打开外部链接前显示确认 |

#### 日志设置

| 设置 | 默认值 | 描述 |
|------|--------|------|
| `inlineMarkdownEditor.log.level` | `"info"` | 日志级别：debug、info、warn、error |
| `inlineMarkdownEditor.log.enableFileLog` | `false` | 启用 JSONL 文件日志 |
| `inlineMarkdownEditor.log.maxFileSizeKB` | `1024` | 轮换前的最大日志文件大小 |
| `inlineMarkdownEditor.log.retentionDays` | `7` | 保留旧日志文件的天数 |

### 📋 命令

| 命令 | 描述 |
|------|------|
| `inlineMarkdownEditor.resetSession` | 重置编辑器会话并重新同步 |
| `inlineMarkdownEditor.applyRequiredSettings` | 应用推荐的工作区设置 |
| `inlineMarkdownEditor.exportLogs` | 导出日志用于调试 |
...
#### 更改未同步
1. 检查输出面板中的错误（查看 > 输出 > Inline Markdown Editor）
